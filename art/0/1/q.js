rbvj = function () {
  ...
  draw = function () {
    ...
    for(var i = 0; i < num; i++) {
        ...
        //stroke(100, 255);
        vel[i].x = 5*noise.perlin3(3 * s + pos[i].x*.007, s + pos[i].y*.007, noisy)*Math.cos(4*Math.PI*noise.perlin3(pos[i].x*.003, pos[i].y*.003, noisy));
        //console.log(vel[i].x);
        vel[i].y = 5*noise.perlin3(3 * s + pos[i].x*.007, s + pos[i].y*.007, noisy)*Math.sin(4*Math.PI*noise.perlin3(pos[i].x*.003, pos[i].y*.003, noisy));
        for(var j = 0; j < num; j++) {
          if(j != i){
